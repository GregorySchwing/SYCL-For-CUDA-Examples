  ...
  do{
    ...
    auto cg2 = [&](sycl::handler &h) {    
      ...
      h.parallel_for(sycl::nd_range<1>{NumWorkItems, WorkGroupSize}, [=](sycl::nd_item<1> item) {
                        ...
                        // Ineligible vertices  are those which are sources at a vertex which has been augmented (match 4+)
                        // or those newly contracted into a blossom (match 3).
                        if (dist_i[base_i[src]] != depth_i[0] || match_i[base_i[src]] > 2) return;
